...
void SceneParser::parsePerspectiveCamera()
{
    ...
    getToken(token);
    if (strcmp(token, "focalLength") == 0) // depth of field
    {
        focalLen = readFloat();
        getToken(token);
    ...
    else if (strcmp(token, "time1") == 0) // motion blur
    {
        time1 = readFloat();
        getToken(token);
        assert(!strcmp(token, "time2"));
        time2 = readFloat();
        getToken(token);
        assert(!strcmp(token, "}"));
    }
    ...
    camera = new PerspectiveCamera(center, direction, up, width, height, angle_radians, focalLen, aperture, time1, time2);
}
...
Material *SceneParser::parseMaterial()
{
    ...
    filename[0] = 0;
    Vector3f diffuseColor(1, 1, 1), specularColor(0, 0, 0), type(1, 0, 0), emissionColor(0);
    ...
    auto *answer = new Material(diffuseColor, emissionColor, specularColor, type, shininess, refr, filename);
    return answer;
}
...
Object3D *SceneParser::parseObject(char token[MAX_PARSER_TOKEN_LENGTH])
{
    ...
    else if (!strcmp(token, "SphereMoving"))
    {
        answer = (Object3D *)parseSphereMoving();
    }
...
SphereMoving *SceneParser::parseSphereMoving()
{
    char token[MAX_PARSER_TOKEN_LENGTH];
    getToken(token);
    assert(!strcmp(token, "{"));
    ...
    assert(!strcmp(token, "}"));
    assert(current_material != nullptr);
    return new SphereMoving(center1, center2, time1, time2, radius, current_material);
}
...
Curve *SceneParser::parseBspline()
{
